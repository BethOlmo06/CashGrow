...
namespace CashGrow.Extensions
{
    public static class InvitationExtensions
    {
        public static async Task SendInvitation(this Invitation invitation)
        {
            ...
            var from = $"CashGrow <{ WebConfigurationManager.AppSettings["emailFrom"]}>";

            var emailMessage = new MailMessage(from, invitation.RecipientEmail)
            {
                Subject = "You have been invited to join CashGrow.",
                Body = $"You can create a new account and join as a member by clicking this link: <1  href=\"{callbackUrl}\">Join</a>" +
                $"<br /><hr />If you have already created an account, copy and past the following code to join the household. Code: {invitation.Code}",
                IsBodyHtml = true
            };
